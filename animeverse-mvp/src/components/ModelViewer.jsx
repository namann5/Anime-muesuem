    ...
    loader.load(
      ...
      loaded => {
        ...
        if (typeof onReady === 'function') {
          const clips = (loaded.animations || []).map((a, i) => ({ name: a.name || `clip ${i}`, index: i }))
          const play = () => { if (currentMixer) currentMixer.timeScale = 1 }
          const pause = () => { if (currentMixer) currentMixer.timeScale = 0 }
          const setClip = idx => {
      ...
      <group ref={group}>
        <group position={[0, 0.6, 0]}>
          ...
          <mesh position={[0, 0.45, 0]}>
            ...
            <meshStandardMaterial color="#ffd7d1" metalness={0.05} roughness={0.6} />
          </mesh>
          ...
          <mesh position={[0, 0.62, -0.02]}>
            <sphereGeometry args={[0.33, 16, 16, 0, Math.PI * 2, 0, Math.PI * 0.6]} />
            <meshStandardMaterial color="#0b1333" metalness={0.1} roughness={0.4} />
          </mesh>
        </group>
      </group>
    )
